...
try {
    ...
    // Pesquisar displayName, userPrincipalName, mail, department e userPassword
    $search = ldap_search($ldap_conn, $ldap_dn, "(objectclass=person)", ["displayName", "userPrincipalName", "mail", "department", "userPassword"]);
    if (!$search) {
...
function syncData(array $entries, PDO $pdo): void
{
    ...
    $stmt_insert = $pdo->prepare("INSERT INTO usuarios (displayName, userPrincipalName, mail, department, userPassword) VALUES (:displayName, :userPrincipalName, :mail, :department, :userPassword)");
    $stmt_update = $pdo->prepare("UPDATE usuarios SET displayName = :displayName, mail = :mail, department = :department, userPassword = :userPassword WHERE userPrincipalName = :userPrincipalName");

    for ($i = 0; $i < $entries["count"]; $i++) {
        // Verificar se as chaves displayName, userPrincipalName, mail, department e userPassword existem
        if (isset($entries[$i]["displayname"][0]) && isset($entries[$i]["userprincipalname"][0]) && isset($entries[$i]["mail"][0]) && isset($entries[$i]["department"][0]) && isset($entries[$i]["userpassword"][0])) {
            ...
            $userPassword = $entries[$i]["userpassword"][0];
